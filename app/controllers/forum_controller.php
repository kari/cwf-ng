...
class ForumController extends AppController {
	...
	function index() {
		$this->redirect("http://".Configure::read("Site.url")."/index.php"); # FIXME: before release
	...
	function latest_topics() {
		if (!isset($this->params["requested"])) { $this->cakeError("error404"); }
		$query = "SELECT Topic.topic_title,Post.post_id,Post.poster_id,User.username FROM phpbb_topics AS Topic LEFT JOIN phpbb_posts AS Post ON Topic.topic_last_post_id = Post.post_id LEFT JOIN phpbb_users AS User ON User.user_id = Post.poster_id ORDER BY Post.post_id DESC LIMIT 10;";
		return $this->Game->query($query);
	}
}